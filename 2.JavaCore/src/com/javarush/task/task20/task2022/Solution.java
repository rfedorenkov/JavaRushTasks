 ...
 * 3. В методе readObject(ObjectInputStream in) не должен быть вызван метод close
 * у потока полученного в качестве параметра.
...
public class Solution implements Serializable, AutoCloseable {
    ...
    public static void main(String[] args) {
        ...
        try (Solution solution = new Solution(filename);
            ...
            solution.writeObject("TEST_ONE");
            oos.writeObject(solution);

            try (Solution loadedSolution = (Solution) ois.readObject()) {
                loadedSolution.writeObject("TEST_TWO");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
