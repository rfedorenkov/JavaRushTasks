...
public class Sportsman {
    ...
    private BigDecimal chanceToHit; // шанс попадания
    private BigDecimal averageSpeed; // средняя скорость
    private BigDecimal resultTime = new BigDecimal("0."); // время результата
    private int number; // номер спортсмена
    ...
    private BigDecimal calculateTimeToRace() {
        return new BigDecimal(RACE_LENGTH / averageSpeed.doubleValue() * 3600)
    ...
    public void printResultInfo() {
        System.out.printf("Спортсмен № %02d: Время - %6.2f секунд. Средняя скорость - %2.4f км/час. Количество промахов %d%n",
